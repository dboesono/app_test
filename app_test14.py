...
TENCENT_VECTORDB_URL = "http://gz-vdb-ev0mnnfu.sql.tencentcdb.com:8100"
...
tencent_vectordb_client = tcvectordb.RPCVectorDBClient(
    ...
    key='SXbbMorbutw7zrOTkLoP0UpiqvRhjaolotyOX74P',
...
def build_vectordb_from_emails(emails):
    ...
    index = Index(
        ...
        VectorIndex(
            name='vector',
            dimension=768,  # Adjust based on your model's dimension
    ...
    embedding_conf = Embedding(vector_field='vector', field='text', model_name='bge-base-zh')
    ...
    for i, email in enumerate(emails):
        ...
        if len(combined_text) > 20000:
            chunks = chunk_text(combined_text, max_length=20000, overlap=500)
    ...
    batch_upsert(docs, batch_size=5, timeout=1000, delay=1)
    # st.info("Vector index built successfully.")
    return collection


def retrieve_top_k_emails(user_query, k=5):
    """
...
def chunk_text(text, max_length=20000, overlap=500):
    """
...
def batch_upsert(docs, batch_size=5, timeout=1000, delay=3):
    """
...
if user_input:
    ...
    if st.session_state.all_emails is None:
        ...
        relevant_emails = retrieve_top_k_emails(user_input, k=15)
